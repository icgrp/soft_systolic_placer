...
void Placer::place() {
    ...
    if (!analytic_place_enabled && !quench_only_ && !placer_opts_.place_algorithm.is_systolic()) {
        // Table header
    ...
    if (placer_opts_.place_algorithm.is_systolic()) {
        // Custom relaxed-SA ("systolic") placer: replaces VPR's anneal+quench.
        ...
        run_systolic(placer_state_, place_delay_model_.get(), placer_criticalities_.get(),
                     placer_setup_slacks_.get(), pin_timing_invalidator_.get(),
                     timing_info_.get(), &costs_, crit_params);
        // Our placer maintains its own cost internally; rebuild VPR's per-net
        // bounding-box cache and bb_cost from scratch (NORMAL repopulates the
        // cache from block_locs) so check_place()/reporting stay consistent.
        // The timing cost is refreshed by the finalization timing update below.
        costs_.bb_cost = net_cost_handler_.comp_bb_cost(e_cost_methods::NORMAL).first;
        costs_.update_norm_factors();
    ...
    if (placer_opts_.place_checkpointing && !placer_opts_.place_algorithm.is_systolic()) {
