...
enum class e_place_algorithm {
    ...
    SLACK_TIMING_PLACE,
    SYSTOLIC_PLACE ///< Relaxed parallel SA (red-black swaps, squared-WL weighted by criticality)
};
  ...
  public:
    ...
    inline bool is_timing_driven() const {
        return algo == e_place_algorithm::CRITICALITY_TIMING_PLACE || algo == e_place_algorithm::SLACK_TIMING_PLACE
               || algo == e_place_algorithm::SYSTOLIC_PLACE;
    }

    ///@brief Check if this is the custom systolic (relaxed-SA) placer.
    inline bool is_systolic() const { return algo == e_place_algorithm::SYSTOLIC_PLACE; }
